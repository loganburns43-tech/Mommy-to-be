...
var CLIENT_HEADERS = [
  ...
  'Household Kids Count',
  'Kids Names/Ages (free text summary or structured string)',
  'Due Month (YYYY-MM or blank)',
  'Baby Born Date (YYYY-MM-DD or blank)',
  'Case Closed Date (YYYY-MM-DD or blank)',
  'Signature Base64 (latest status signature)',
...
function listClients() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    list.push({
      ...
      status: status,
      lastActivity: String(row[13] || '').trim()
    });
  }
  return list;
}
...
function createClient(payload) {
  ...
  try {
    ...
    var kidsCount = cleanText(payload.kidsCount);
    var kidsInfo = cleanText(payload.kidsInfo);
...
function buildClientObject(row) {
  return {
    ...
    kidsCount: String(row[7] || ''),
    kidsInfo: String(row[8] || ''),
    dueMonth: String(row[9] || ''),
    babyBornDate: String(row[10] || ''),
    caseClosedDate: String(row[11] || ''),
    signature: String(row[12] || ''),
    lastActivity: String(row[13] || ''),
...
function saveProvision(payload) {
  ...
  try {
    ...
    sheet.getRange(info.rowNumber, 14).setValue(now);
    if (signature) {
      sheet.getRange(info.rowNumber, 13).setValue(signature);
    }
    appendLedger({
      timestamp: now,
      clientId: clientId,
...
function closeCase(payload) {
  ...
  try {
    ...
    var now = new Date();
    var bornDate = cleanText(payload.babyBornDate);
    var signature = cleanText(payload.signature);
    sheet.getRange(info.rowNumber, 5).setValue('Closed');
    sheet.getRange(info.rowNumber, 6).setValue(reason);
    sheet.getRange(info.rowNumber, 11).setValue(bornDate);
    sheet.getRange(info.rowNumber, 12).setValue(now);
    if (signature) {
      sheet.getRange(info.rowNumber, 13).setValue(signature);
    }
    sheet.getRange(info.rowNumber, 14).setValue(now);
