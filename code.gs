...
function listClients() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    list.push({
      ...
      status: status,
      lastActivity: String(row[10] || '').trim()
    });
  }
  return list;
}
...
function buildClientObject(row) {
  return {
    ...
    babyBornDate: String(row[8] || ''),
    signature: String(row[9] || ''),
    lastActivity: String(row[10] || ''),
...
function saveProvision(payload) {
  ...
  try {
    ...
    sheet.getRange(info.rowNumber, 11).setValue(now);
    if (signature) {
      sheet.getRange(info.rowNumber, 10).setValue(signature);
    }
    appendLedger({
      timestamp: now,
      clientId: clientId,
...
function closeCase(payload) {
  ...
  try {
    ...
    sheet.getRange(info.rowNumber, 9).setValue(bornDate);
    if (signature) {
      sheet.getRange(info.rowNumber, 10).setValue(signature);
    }
    sheet.getRange(info.rowNumber, 11).setValue(now);
